...
function applySettings(){ statusDiff.textContent = difficultySelect.value; statusSound.textContent = soundToggle.value === 'on' ? 'On' : 'Off'; soundEnabled = soundToggle.value === 'on'; // blocks
    blocksActive = toggleBlocks && toggleBlocks.checked; blocksCount = blocksCountInput ? parseInt(blocksCountInput.value,10)||3 : 3; if (blocksActive) generateBlocks(); else blocks = []; }

// generate random blocks avoiding paddles and center
function generateBlocks(){ blocks = []; const w = Math.max(28, Math.floor(canvas.width * 0.06)); const h = Math.max(18, Math.floor(canvas.height * 0.06)); const padding = 20; const maxAttempts = 300; let attempts = 0; while(blocks.length < blocksCount && attempts++ < maxAttempts){ const x = Math.floor(padding + Math.random() * (canvas.width - padding*2 - w)); const y = Math.floor(padding + Math.random() * (canvas.height - padding*2 - h)); // avoid near player/AI paddles areas
    ...
    if (!ok) continue; blocks.push({x,y,w,h}); }
}
...
// blocks UI listeners
if (toggleBlocks) toggleBlocks.addEventListener('change', e => { saveSettings(); });
if (blocksCountInput) blocksCountInput.addEventListener('change', e => { blocksCount = parseInt(e.target.value,10) || 3; saveSettings(); });
...
function updateBall(){ if (!ball) return; ball.x += ball.vx; ball.y += ball.vy;
    ...
    // blocks collision
    if (toggleBlocks && toggleBlocks.checked && blocks.length){
        for (const b of blocks){
            if (ball.x + BALL_RADIUS > b.x && ball.x - BALL_RADIUS < b.x + b.w && ball.y + BALL_RADIUS > b.y && ball.y - BALL_RADIUS < b.y + b.h){
                // determine smaller penetration axis
                const px = Math.min(ball.x + BALL_RADIUS - b.x, b.x + b.w - (ball.x - BALL_RADIUS));
                const py = Math.min(ball.y + BALL_RADIUS - b.y, b.y + b.h - (ball.y - BALL_RADIUS));
                if (px < py){
                    // reflect horizontally
                    if (ball.x < b.x) ball.x = b.x - BALL_RADIUS; else ball.x = b.x + b.w + BALL_RADIUS;
                    ball.vx *= -1.05;
                } else {
                    // reflect vertically
                    if (ball.y < b.y) ball.y = b.y - BALL_RADIUS; else ball.y = b.y + b.h + BALL_RADIUS;
                    ball.vy *= -1.05;
                }
                playHit();
                break;
...
function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height);
    ...
    // blocks
    if (toggleBlocks && toggleBlocks.checked && blocks.length){ for (const b of blocks){ drawRect(b.x, b.y, b.w, b.h, '#666'); } }
    // paddles
